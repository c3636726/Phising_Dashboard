...
packages <- c("shiny", "shinydashboard", "dplyr", "tidyverse", "mice", "ggplot2", 
              "pROC", "class", "e1071", "rpart", "rpart.plot", "reshape2", 
              "randomForest", "ROCR", "caret", "corrplot")
...
server <- function(input, output) {
  ...
  output$corrplot <- renderPlot({
    tryCatch({
      corrplot(cor(data_cleaned[sapply(data_cleaned, is.numeric)]), method = "color")
    }, error = function(e) {
      plot.new()
      text(0.5, 0.5, "Correlation plot error: Check corrplot package", cex = 1.5)
    })
  })
